...
class StereographicallyProjectedSphere(RadiusManifold):
    ...
    def exp_map(self, x, at_point):
        return exp_map(x=x, at_point=at_point, radius=self.radius)
...
def poincare_distance_mat(x: Tensor, y: Tensor, radius) -> Tensor:
    k = _k(radius)
    dis = torch.cdist(x, y)
    x_norm_term = 1. + k * torch.norm(x, dim=-1).pow(2)
    y_norm_term = 1. + k * torch.norm(y, dim=-1).pow(2)
    dis = (dis / y_norm_term).t()
    dis = (dis / x_norm_term).t()
    dis = radius * torch.acos(torch.clamp(1- 2 * k * dis, min = 1e-8 - 1, max = 1 - 1e-8))
    return dis
